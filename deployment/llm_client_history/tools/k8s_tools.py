...
class LokiLogsQueryRoutine(BaseTool):
    """
    Tool for automatically retrieving non-info logs from Loki for each label-value combination.
    
    This tool queries Loki for all available label-value pairs (excluding 'filename'),
    then fetches logs for each combination from the last 30 minutes, filtering out
    logs containing 'info'. It's useful for getting a quick overview of warning,
    error, and critical logs across multiple services without having to construct
    individual queries.
    
    No parameters are required as the tool automatically:
    1. Fetches all label values from Loki
    2. Constructs LogQL queries for each label-value pair
    3. Retrieves logs from the last 30 minutes
    4. Filters out informational logs to focus on potential issues
    5. Aggregates results by label and value
    
    This is particularly useful for initial diagnostics or periodic system health checks.
    """
    name: str = "loki_logs_routine"
    description: str = ("Automatically retrieve non-info logs from all services monitored by Loki. "
                       "This tool fetches logs from the last 30 minutes for all label-value pairs, "
                       "filtering out logs containing 'info' to focus on warnings, errors, and critical issues. "
                       "No parameters needed - it automatically queries across all services and components. "
                       "Results are grouped by label and value for easy navigation. "
                       "Use this for quick system health checks or initial troubleshooting.")
    
    def _run(self) -> Dict[str, Any]:
        """
        Query the Loki API for logs across all label-value combinations, excluding info logs.
        
        Returns:
            Dict[str, Any]: Dictionary containing:
                - status: 'success' or 'error'
                - logs: Aggregated logs data organized by label and value (on success)
                - error: Error message (on failure)
                
        Note:
            This routine fetches logs from the last 30 minutes only and filters out info-level logs
        """
        try:
            # 發送請求到 Loki API
            response = requests.get(
                f"{settings.loki_api_url}/logs_routine",
                timeout=30
            )
            ...
            return {
                "status": "error",
                "error": f"Error occurred when getting the routine's logs: {str(e)}"
            }
